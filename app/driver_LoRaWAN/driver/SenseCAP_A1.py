...
def run(payload, identifier):
    """Driver for sensor SenseCAP A1

    This driver extracts from the payload the value observed by the sensor SenseCAP,
    (first sensor version) it is a soil temperature and humidity sensor
    ...
    if len(payload) == 32:
        ...
        if payload[16:20] == identifier:
            value_raw = struct.unpack_from('<I', payload_bytes, 10)[0]
            decoded_data[identifier] = value_raw / 1000

        elif payload[30:34] == identifier:
            value_raw = struct.unpack_from('<I', payload_bytes, 17)[0]
            decoded_data[identifier] = value_raw / 1000

        elif identifier == 'battery' and payload[2:6] == "0700":
            battery_raw = struct.unpack_from('<H', payload_bytes, 3)[0]
...
EXAMPLE_PAYLOAD = "010610cb180000010710ae9200005ad7"
